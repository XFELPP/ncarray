...
namespace ncarray {
  namespace host {
    ...
    RuntimeCompiler& RuntimeCompiler::instance() {
      ...
      // need this change
      mallopt(M_MMAP_MAX, 0);                        // Disable mmap for heap allocation
      mallopt(M_MMAP_THRESHOLD, 1024 * 1024 * 1024); // 1 GB
      mallopt(M_TRIM_THRESHOLD, -1);                 // Disable trim/madvise returning to OS
                                                     // - Subsequent allocations reuse pages
      // NOTE: The side-effect is that RSS (resident set size) grows until peak
      //       memory usage since pages aren't returned to the OS.
      //       Performance benefit comes from avoiding the mapping/unmapping of pages
      //       But, if the memory usage is high (or there's a leak...) this could be
      //       separately problematic.
      // TODO: Consider making this configurable? Default to true, env var to disable?
#endif
      ...
      if (!fs::exists(cache_path)) {
        ...
        if (env.is_arch_x64()) {
          ...
          compile_x86_expr_kernel(cache_path,
                                  ...
                                  scalars,
                                  expr_is_soarr);
        } else if (env.is_arch_aarch64()) {
          // 64-bit AArch64 (ARM)
          compile_a64_expr_kernel(cache_path,
                                  dest_t,
                                  src_t,
                                  work_t,
                                  dest_layout,
        ...
        // Don't handle other cases currently
      }
    ...
    void RuntimeCompiler::compile_a64_expr_kernel(fs::path cache_path,
                                                  DType dest_t,
                                                  DType src_t,
                                                  DType work_t,
                                                  const SOArrayPolicy& dest_layout,
      ...
      asmjit::a64::Gp src_ptrs_reg { cc.new_gp_ptr("src_ptrs") };
      asmjit::a64::Gp dest_ptr_reg { cc.new_gp_ptr("dest_ptr") };
      ...
      //       - Pull out the base pointer for each array view
      //       - Dereference and propagate offset at each level of the loop as you go
      // NOTE: Finally, in the innermost loop, a64 supports hardware scaling when
      //       you address a pointer like [base + index * scale] and scale is 1,2,4,8
      std::vector<asmjit::a64::Gp> base_ptrs(instrs.size());
      for (const auto& instr : instrs) {
        OpCode op { get_op(instr) };
        if (op != OpCode::LOAD_NCARR && op != OpCode::LOAD_SOARR) {
          continue;
        ...
        cc.ldr(base_ptrs[idx], asmjit::a64::ptr(src_ptrs_reg, idx * sizeof(void*)));
      }

      // Hold the pointer for the view up to dimension `dim`: op_ptrs[view_idx][dim]
      std::vector<std::vector<asmjit::a64::Gp>> op_ptrs(instrs.size(),
                                                        std::vector<asmjit::a64::Gp>(ndim));

      // Likewise, track the destination pointer accumulation
      std::vector<asmjit::a64::Gp> dest_ptrs(ndim);

      // Setup loop limits and load into registers
      std::vector<asmjit::a64::Gp> loop_limits;
      // Setup loop indexing registers
      std::vector<asmjit::a64::Gp> loop_regs;
      // Setup loop labels and end points
      std::vector<asmjit::Label> loop_labels;
      std::vector<asmjit::Label> loop_ends;
      for (ssize_t dim = 0; dim < ndim; ++dim) {
        ...
        // Setup a nested loop: Outer Dim --> Inner Dim --> ... --> Innermost
        cc.mov(loop_regs[dim], 0);
        if (dim == ndim - 1) {
          cc.align(asmjit::AlignMode::kCode, 16);
        }
        cc.bind(loop_labels[dim]);
        cc.cmp(loop_regs[dim], loop_limits[dim]);
        cc.b_ge(loop_ends[dim]);
        ...
        if (dim < ndim - 1) {
          ...
          if (!expr_is_soarr) {
            ...
            if (ncl.is_pointer_axis(dim)) {
              // For NCOffsetsPolicy for pointer axes we derefence with [index + offset]
              a64::advance_ncoffsets_pointer_axis(cc,
                                                  dest_ptrs[dim],
                                                  loop_regs[dim],
                                                  ncl.offset(dim));
            } else {
              // Normal strided traversal (index * stride + offset)
              a64::advance_ncoffsets_strided_axis(cc,
                                                  dest_ptrs[dim],
                                                  loop_regs[dim],
                                                  ncl.stride(dim),
                                                  ncl.offset(dim));
      ...
      for (const auto& instr : instrs) {
        ...
        if (op == OpCode::LOAD_NCARR || op == OpCode::LOAD_SOARR) {
          asmjit::Reg v_reg { cc.new_reg<asmjit::Reg>(type_id) };
          asmjit::a64::Gp val_addr { cc.new_gp_ptr() };
          ...
          // Load using the source type, but cast to our working type
          asmjit::Reg casted_reg {
            a64::cast_register(cc, v_reg, src_type_id, type_id)
          };

          // Push to stack and advance
          reg_stack.push_back(casted_reg);
        } else if (op == OpCode::LOAD_CONST) {
          asmjit::Reg s_reg { a64::load_constant(cc, scalars[idx], type_id) };
          reg_stack.push_back(s_reg);
        } else if (op == OpCode::IDX) {
          asmjit::a64::Gp lin_idx_reg { cc.new_gp(asmjit::TypeId::kInt64) };
          cc.mov(lin_idx_reg, 0);
          for (ssize_t dim = 0; dim < ndim; ++dim) {
            ssize_t stride { cum_nelem_strides[dim] };
            if (stride == 1) {
              cc.add(lin_idx_reg, lin_idx_reg, loop_regs[dim]);
            } else {
              ...
              cc.mul(term, loop_regs[dim], stride_reg);
              cc.add(lin_idx_reg, lin_idx_reg, term);
            }
          }

          // Convert the IDX ravel index to the working type and push to stack
          asmjit::Reg v_reg {
            a64::cast_register(cc, lin_idx_reg.as<asmjit::Reg>(), asmjit::TypeId::kInt64, type_id)
          };
          reg_stack.push_back(v_reg);
        } else if (static_cast<int>(op) < static_cast<int>(OpCode::ADD)) {
          // NOTE: OpCodes are ordered, so comparisons of value work.
          ...
          // Allocate new register for result and perform operation
          asmjit::Reg res { cc.new_reg<asmjit::Reg>(type_id) };
          a64::unary_operation(cc, op, type_id, operand, res);
          ...
          if (static_cast<int>(op) < static_cast<int>(OpCode::EQ)) {
            ...
            if (asmjit::TypeUtils::is_int(type_id) && (op == OpCode::DIV || op == OpCode::MOD)) {
              // Integer division and modulo has to be done in specific-registers
              a64::integer_div_mod(cc, op, type_id, l, r, res);
            } else {
              // on A64, use 3-operand binary instructions: res = l op r
              cc.emit(a64::get_binary_arithmetic_inst(op, type_id), res, l, r);
            }
          } else if (static_cast<int>(op) < static_cast<int>(OpCode::LAND)) {
            // Comparisons
            a64::binary_compare(cc, op, type_id, l, r, res);
          }
          reg_stack.push_back(res);
        }
      }
      ...
      asmjit::TypeId dest_type_id { dtype_to_typeid(dest_t) };
      asmjit::Reg casted_res { a64::cast_register(cc, final_res, type_id, dest_type_id) };

      // Resolve the location for storing the result in the destination
      asmjit::a64::Gp dest_addr { cc.new_gp_ptr("dest_addr") };
      if (ndim > 1) {
        cc.mov(dest_addr, dest_ptrs[ndim - 2]);
      } else {
        cc.mov(dest_addr, dest_ptr_reg);
      ...
      // NOTE: Go from the innermost --> out!
      for (ssize_t dim = ndim - 1; dim >= 0; --dim) {
        cc.add(loop_regs[dim], loop_regs[dim], 1);
        cc.b(loop_labels[dim]);
        cc.bind(loop_ends[dim]);
      }
